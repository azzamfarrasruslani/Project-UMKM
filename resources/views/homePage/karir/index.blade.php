<x-guest-layout>
    ...
    <section class="flex flex-col mt-30 w-full tracking-tight max-md:max-w-full" aria-labelledby="delivery-menu-title">
        ...
        <section class="flex flex-col self-center px-4 w-full max-w-[1336px] min-h-[689px] max-md:max-w-full mt-14"
            ...
            <header data-aos="zoom-in"  class="mb-8 text-center">
            <h2 class="text-3xl font-bold text-gray-900">Peluang Karir Tersedia</h2>
            <p class="mt-2 text-lg text-gray-600">Jelajahi berbagai peluang karir yang tersedia dan bergabunglah dengan tim kami untuk masa depan yang lebih baik.</p>
            </header>
            ...
            @if (!empty($karir) && !is_array($karir))
                ...
                <article data-aos="fade-up"
                    ...
                    <div class="flex items-center space-x-2">
                        <span class="inline-block px-2 py-1 text-xs font-semibold text-white bg-customRed-100 rounded">{{ $item->tipe_kerja }}</span>
                    </div>
                    ...
                    <div class="mt-2 flex space-x-4 text-sm text-gray-500">
                        ...
                        <p><i class="fa-regular fa-clock mr-2"></i> Posted on: {{ $item->created_at->format('M d, Y') }}</p>
                    </div>
                    @if ($item->gaji)
                        <p class="mt-2 text-gray-600"><i class="fa-solid fa-money-bill-wave mr-2"></i> Gaji: Rp{{ number_format($item->gaji, 0, ',', '.') }}</p>
                    @endif
                    @if ($item->benefit)
                        <p class="mt-2 text-gray-600"><i class="fa-solid fa-gift mr-2"></i> Benefit: {{ $item->benefit }}</p>
                    @endif
                    <p class="mt-2 text-gray-600"><i class="fa-solid fa-info-circle mr-2"></i> Status: {{ ucfirst($item->status) }}</p>
                    </div>
