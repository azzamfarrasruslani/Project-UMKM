<x-app-layout>
    <div class="max-w-4xl mx-auto py-6">
        <div class="bg-white p-6 shadow-md rounded-lg">
            <h1 class="text-2xl font-bold mb-6 text-center">Tambah Pesan Saran</h1>
            ...
            <form action="{{ route('pesansaran.store') }}" method="POST">
                ...
                <div class="mb-6">
                    <textarea name="pesan" rows="4" placeholder="Pesan"
                        class="w-full border-gray-300 rounded-lg p-3 focus:ring focus:ring-blue-300">{{ old('pesan') }}</textarea>
                    @error('pesan')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
