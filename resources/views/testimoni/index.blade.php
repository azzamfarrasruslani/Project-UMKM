                ...
                <div
                    ...
                    <div class="relative z-10 p-9">
                        <div class="flex justify-between items-center">
                            <h1 class="text-4xl font-extrabold  text-white">Data Testimoni</h1>
                            <a href="javascript:;" onclick="openAddTestimoni()"
                        ...
                        <div id="kandangTabContent"
                            ...
                            @foreach ($testimoni as $index => $item)
                                <div class="card-kandang hidden" data-tab="tab-{{ ceil(($index + 1) / 6) }}">
                                    <div
                                        class="bg-white text-slate-700 shadow-md rounded-lg overflow-hidden text-center p-6  ">

                                        <!-- Pesan Review -->
                                        <div class="mb-4">
                                            <p class="text-lg italic text-gray-700">"{{ $item->pesan }}"</p>
                                        </div>
                                        <!-- Nama Reviewer -->
                                        <div class="font-bold text-xl mb-2 text-red-600">
                                            {{ $item->nama }}
                                        </div>
                                        <!-- Pekerjaan Reviewer -->
                                        <div class="text-sm text-gray-500 mb-4">
                                            {{ $item->asal_testimoni ?? 'User' }}
                                        </div>
                                        ...
                                        <div class="flex justify-center space-x-2">
                                            ...
                                            <form id="deleteForm" action="{{ route('testimoni.destroy', $item->id_testimoni) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                                <a href="javascript:;" type="button" data-id="{{ $item->id_testimoni }}"
                                                    data-url="{{ route('testimoni.destroy', $item->id_testimoni) }}"
